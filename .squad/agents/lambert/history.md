...
### 2026-04-30T23:20:49+02:00 — Conversation Management UI
- **Feature:** Full conversation management flow — menu landing page, resume saved conversations, back navigation
- **Architecture:** index.html now has two views (menu vs settings form) toggled via `.hidden` class. Menu shows "New Conversation" button + list of saved conversations from localStorage.
- **Persistence:** New localStorage key `voiceAgentConversations` stores array of `{conversationId, settings, agentName, date, label}`. Saved when backend sends `{type:"conversation_id", id:"..."}` via WebSocket.
- **Resume flow:** Clicking "Resume" on a saved conversation loads its full settings + conversationId into `voiceAgentConfig` and navigates to voice.html. The `conversation_id` field is sent in the config WS message.
- **Back navigation:** "Back to Menu" button appears after session ends (ws close, error, or user click "End Session"). Returns user to index.html menu.
- **Delete:** Each saved conversation card has a delete button (✕) to remove it from localStorage.
- **Styling:** New CSS for `.conversation-card`, `.menu-actions`, `.conversation-list`, `.hidden` utility class. Consistent with existing dark theme.
- **Key files modified:** `static/index.html`, `static/js/config.js`, `static/js/voice.js`, `static/css/style.css`
